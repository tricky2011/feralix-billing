    ...
    private function resolveRouters()
    {
        ...
        return Router::query()
            ...
            ->whereHas('services', function ($query): void {
                // Monitor can match via monitor_pppoe_username OR via pppoe_username
                // (when monitor_pppoe_username is NULL, sync falls back to pppoe_username).
                $query->where(function ($q): void {
                    $q->whereNotNull('monitor_pppoe_username')
                      ->where('monitor_pppoe_username', '!=', '');
                })->orWhere(function ($q): void {
                    $q->whereNotNull('pppoe_username')
